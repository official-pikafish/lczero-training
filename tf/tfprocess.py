...
class TFProcess:
    ...
    def process_inner_loop(self, x, y, z, q, m, st_q, opp_probs, next_probs, fut):
        ...
        with tf.GradientTape() as tape:
            ...
            value_q_loss, value_q_err_loss, value_q_cat_loss = self.value_losses_fn(
                q, value_q, value_q_err, value_q_cat) if value_q is not None else (
                tf.constant(0.), tf.constant(0.), tf.constant(0.))
            value_st_loss, value_st_err_loss, value_st_cat_loss = self.value_losses_fn(
                st_q, value_st, value_st_err, value_st_cat) if value_st is not None else (
                tf.constant(0.), tf.constant(0.), tf.constant(0.))
            if self.wdl:
    ...
    def scaled_dot_product_attention(self, q, k, v, name: str = None, inputs=None):
        ...
        if self.use_smolgen:
            smolgen_weights = self.smolgen_weights(inputs, heads, self.smolgen_hidden_channels, self.smolgen_hidden_sz,
                                                   self.smolgen_gen_sz, name=name + "/smolgen",
    ...
    def mha(self, inputs, emb_size: int, d_model: int, num_heads: int, initializer, name: str):
        ...
        output = tf.keras.layers.Dense(
            emb_size, name=name + "/dense", kernel_initializer=initializer, use_bias=not self.omit_other_biases)(
            scaled_attention)
    ...
    def ffn(self, inputs, emb_size: int, dff: int, initializer, name: str):
        ...
        dense1 = tf.keras.layers.Dense(
            dff, name=name + "/dense1", kernel_initializer=initializer, activation=activation,
            use_bias=not self.omit_other_biases)(inputs)

        out = tf.keras.layers.Dense(
            emb_size, name=name + "/dense2", kernel_initializer=initializer, use_bias=not self.omit_other_biases)(
            dense1)
        return out
    ...
    def smolgen_weights(self, inputs, heads: int, hidden_channels: int, hidden_sz: int, gen_sz: int, name: str,
                        activation="swish"):
    ...
    def construct_net(self, inputs, name: str = ""):
        ...
        if self.use_smolgen:
            self.smol_weight_gen_dense = tf.keras.layers.Dense(
                90 * 90, name=name + "smol_weight_gen", use_bias=False)
        ...
        if self.embedding_style == "new":
            ...
            pos_info_processed = tf.keras.layers.Dense(
                90 * self.embedding_dense_sz, name=name + "embedding/preprocess")(pos_info_flat)
            pos_info = tf.reshape(pos_info_processed,
                                  [-1, 90, self.embedding_dense_sz])
        ...
        def policy_head(name, activation=None, depth=None, opponent=False):
            ...
            # Bx90x90 (from 90 queries, 90 keys)
            ...
            # Bx90x90 (90 from-squares, 90 to-squares)
            policy_attn_logits = matmul_qk / dk

            attn_wts.append(policy_attn_logits)

            # APPLY POLICY MAP: output becomes Bx2062
            h_fc1 = ApplyAttentionPolicyMap(name=name + "/attention_map")(policy_attn_logits)
        ...
        value_q, value_q_err, value_q_cat = value_head(
            name="value/q", wdl=self.wdl, use_err=True) if self.cfg['model'].get('value_q', False) else (
            None, None, None)
        value_st, value_st_err, value_st_cat = value_head(
            name="value/st", wdl=self.wdl, use_err=True) if self.cfg['model'].get('value_st', False) else (
            None, None, None)

        def future_head(name):
            fut = tf.keras.layers.Dense(15 * self.n_future_boards,
                                        kernel_initializer="glorot_normal",
                                        name=name + "/dense")(flow)
            fut = tf.reshape(fut, [-1, 90, self.n_future_boards, 15])
            return fut
